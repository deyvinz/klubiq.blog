...
export async function fetchStrapiData<T>(endpoint: string, params: StrapiQueryParams = {}): Promise<T> {
  try {
...
export async function getAboutPage(): Promise<StrapiAboutPage> {
  const response = await fetchStrapiData<StrapiResponse<StrapiAboutPage>>('/about-page', {
    populate: {
      seo: true,
      vision: true,
      blog: true,
